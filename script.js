...
document.addEventListener("DOMContentLoaded",(evt)=>{
  ...
  for (hero in heroes){
    ...
    <p class="hero__universe"><h3>"Вселенная: "</h3>` + `${hero.universe}</p>` +
    `<p class="hero__alterEgo"><h3>"Альтер эго: "</h3>` + `${hero.work}</p>` +
    `<p class="hero__universe"><h3>"Род деятельности: "</h3>` + `${hero.friend}</p>` +
    `<p class="hero__superpower"><h3>"Друзья: "</h3>` + `${hero.superpower}</p>` +
    `<img class="hero__img" />${hero.img.src}
    </div>`
  }

  document.querySelector(".heroes_gallery").innerHTML = hero;
})
